...
public class EnemyComponent : MonoBehaviour {
	...
	// 	float angleOffset = 5;
	// 	Vector3 offset = Quaternion.Euler(0, Random.Range(-angleOffset, angleOffset), 0) * velocity;

	// 	if(velocity == Vector3.zero){
	// 		rigidbody.AddForce(new Vector3(Random.Range(-1f, 1), 0, Random.Range(-1f, 1)));
	// 	}

	// 	direction = transform.position + velocity + offset;
