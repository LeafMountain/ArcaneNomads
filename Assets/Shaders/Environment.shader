Shader "TreeBranch/Environment" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			fixed4 noiseH = tex2D(_NoiseTex, IN.worldPos.xz * _NoiseScale);
			fixed4 decalH = tex2D(_DecalTex, IN.worldPos.xz * _DecalScale) * _DecalColor;

			float3 xd = tex2D(_DecalTex, IN.worldPos.zy * _DecalScale);
			float3 zd = tex2D(_DecalTex, IN.worldPos.xy * _DecalScale);
			float3 yd = tex2D(_DecalTex, IN.worldPos.zx * _DecalScale);
